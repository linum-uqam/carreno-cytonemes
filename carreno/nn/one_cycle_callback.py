...
# Code is mostly copied from https://github.com/titu1994/keras-one-cycle/blob/master/clr.py
# commit a453b749140da9f3581c445ba353175242d3bcbd
...
class LRFinder(Callback):
    def __init__(self,
        ...
        # Arguments:
            ...
            validation_data: Requires the validation dataset as a tf.keras.utils.Sequence
                belonging to the validation set. If provided, will use the
        ...
        self.num_batches_ = len(self.validation_data)
        ...
        if lr_scale == 'exp':
            ...
            self.lr_multiplier_ = np.linspace(
                minimum_lr, maximum_lr, num=self.num_batches_)

        # If negative, use entire validation set
        if self.validation_sample_rate < 0:
            self.validation_sample_rate = len(self.validation_data)
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self.use_validation_set:
            # use N random batches from test set for fast approximate of loss
            num_samples = min(self.validation_sample_rate, len(self.validation_data))
            self.validation_data.on_epoch_end() # shuffle
            values = self.model.evaluate(x=self.validation_data,
