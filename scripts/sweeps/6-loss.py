...
def training():
    ...
    loss_fn = mtc.dice_loss
    if loss == "bce_dice":
        loss_fn = mtc.bce_dice_loss
    if loss == "focal":
        loss_fn = mtc.focal_loss
    if loss == "focal_bce_dice":
        loss_fn = mtc.focal_bce_dice_loss
    if loss == "cldice":
        if is_2D:
            loss_fn = mtc.dice_cldice2D_loss()
        else:
            loss_fn = mtc.dice_cldice3D_loss()
