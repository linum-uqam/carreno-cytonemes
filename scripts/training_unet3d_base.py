...
out_model_name = "unet3d_base.h5"
out_model_dir  = config['DIR']['model']
wdb_project    = 'unet3d_base'

params = {
    'ndim'     : 3,              # 2 or 3
    'shape'    : [48, 80, 80],   # data shape
    'depth'    : 4,              # unet depth
    ...
    'dupe'     : 1               # nb d'usage pour un volume dans une époque
}
...
def main(verbose=0):
    ...
    with open(pickle_path, 'wb') as f:  # open a text file
        pickle.dump(model, f, protocol=None, *, fix_imports=True, buffer_callback=None)
