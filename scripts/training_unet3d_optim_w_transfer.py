...
out_model_name = "unet3d_optim_w_transfer3.h5"
wdb_project    = 'unet3d_optim_w_transfer3'
out_model_dir  = config['DIR']['model']
path_2_unet2d  = "D:/Etude/LINUM/resultat/unet2d_optim_w_unbaleled_v8/unet2d_optim_w_unlabeled8.h5"
...
def main(verbose=0):
    ...
    print("#####################")
    print("# TRANSFER 2D TO 3D #")
    print("#####################")

    # load model
    model2D.load_weights(path_2_unet2d)
    model3D = model2D_to_3D(model2D, 16)
    ...
    fullpath = lambda dir, files : [os.path.join(dir, name) for name in files] * max(1, params['dupe'] // 2)
    ...
    def compile_n_fit(train_gen, valid_gen, start_epoch, log=True):
        ...
        # reset epoch & optim to early stop
        optim.set_weights(model.optimizer.get_weights())
        return hist, max(0, final_epoch - patience - 1)
